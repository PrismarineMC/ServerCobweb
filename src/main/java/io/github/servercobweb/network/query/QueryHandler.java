package io.github.servercobweb.network.query;
...
public class QueryHandler {
    ...
    public QueryHandler() {
        ...
        this.server.getLogger().info(this.server.getLanguage().translateString("nemisys.server.query.start"));
        String ip = this.server.getIp();
        String addr = (!"".equals(ip)) ? ip : "0.0.0.0";
        int port = this.server.getPort();
        this.server.getLogger().info(this.server.getLanguage().translateString("nemisys.server.query.info", String.valueOf(port)));

        this.regenerateToken();
        this.lastToken = this.token;
        this.regenerateInfo();
        this.server.getLogger().info(this.server.getLanguage().translateString("nemisys.server.query.running", new String[]{addr, String.valueOf(port)}));
    }
